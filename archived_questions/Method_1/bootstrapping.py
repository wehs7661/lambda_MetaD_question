...
def initialize():
    ...
    parser.add_argument('-i',
                        '--input',
                        default='plumed_final_bias.dat',
                        help='The PLUMED input file to calculate the final bias. \
                              Default: plumed_final_bias.dat')
    parser.add_argument('-c',
                        '--colvar',
                        default=glob.glob('COLVAR*'),
                        help='The COLVAR file (could be preprocessed) to be analyzed. ')
...
if __name__ == '__main__':
    ...
    for folder in args.dir:
        t1 = time.time()
        if len(args.colvar) == 1:
            colvar = folder + str(args.colvar[0])
        elif type(args.colvar) == str:
            colvar = folder + args.colvar
        ...
        L.logger(
            f'- Files analyzed/used: {args.input}, {args.colvar}, {args.hills}')
        ...
        # Step 2: Calculate the free energy and its uncertainty
        ...
        hills_data = plumed.read_as_pandas(hills)
        traj["metad.bias"] = metad_bias(average_bias(hills_data, avg_frac))[
            np.int_(traj["lambda"])]

        # number of data points considered
        n = int(len(traj) * (1 - truncate))
        ...
        L.logger(
            f'\nFiles output by this code: fes*dat, COLVAR_FINAL_BIAS, df_err_bsize_truncate_{truncate}_avg_{avg_frac}.png, {result_file.split("/")[-1]}')
        t2 = time.time()
